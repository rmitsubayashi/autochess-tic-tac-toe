...
class AnimateAttack(private val uiBoard: UIBoard): Action(EmptyEventActor()) {
    ...
    override fun execute(game: Game, event: Event, userInputResult: List<EventActor>?): List<Event> {
        ...
        game.animationQueue.addAnimation(attackingUIPiece, Actions.sizeBy(10f, 10f, 0.2f))
        game.animationQueue.addAnimation(attackingUIPiece, Actions.moveBy(-destX, -destY, 0.5f, Interpolation.slowFast))
        game.animationQueue.addAnimation(attackingUIPiece, Actions.moveBy(destX, destY, 0.5f, Interpolation.fastSlow))
        game.animationQueue.addAnimation(attackingUIPiece, Actions.sizeBy(-10f, -10f, 0.2f))
